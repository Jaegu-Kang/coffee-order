 ...
 * {@code type}은 {@code VARCHAR(10)}(CHARGE / USE) 컬럼으로, {@link PointHistoryType}
 * enum을 {@code @Enumerated(EnumType.STRING)}으로 매핑해 문자열로 저장한다. {@code point_histories}
 * 테이블에는 {@code updated_at} 컬럼이 없으므로 이 엔티티에는 감사 필드로 {@code createdAt}만 둔다.
...
public class PointHistory {
	...
	public static final PointHistoryType TYPE_USE = PointHistoryType.USE;
