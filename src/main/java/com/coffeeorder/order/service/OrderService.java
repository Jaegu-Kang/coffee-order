 ...
 * 1~2, 4~5단계(존재확인 → 총액 계산 → 포인트 차감/이력 기록 → 주문·주문항목 저장)를
 * 하나의 트랜잭션으로 처리한다(SCRUM-55).
 * <p>
 * 3단계(잔액 조회 시 비관적 락)·다중 인스턴스 동시 주문을 막는 Redis 분산락·6단계
 * (커밋 후 Kafka 이벤트 발행)는 도전 과제(E6) 범위이며 이번 티켓에서는 다루지 않는다
 * (docs/design/jira-backlog.md).
 */
@Service
